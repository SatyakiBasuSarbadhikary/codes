...
from the three methods of cross validation,the k fold cross validation approach yields the minimum average test mse for k=10(14.4859)
now the validation set approach yielded largest test mse (24.922)and on repeatedly performing the approach 50 times gave us a relatively smaller test mse(15.483)while leave one out approach yielded 13.8985 average test mse
Leave one out approach gives better predictions.
...
library("caret")
#K FOLD CROSS VALIDATION WHEN 5 FOLDS
ctrl5 <- trainControl(method = "cv", number = 5)
model5 <- train(stack.loss~Air.Flow+Water.Temp+Acid.Conc., data = stackloss,method ="lm", trControl = ctrl5)
#print(model)

#K FOLD CROSS VALIDATION WITH 7 FOLDS
ctrl7 <- trainControl(method = "cv", number = 7)
model7 <- train(stack.loss~Air.Flow+Water.Temp+Acid.Conc., data = stackloss,method ="lm", trControl = ctrl7)

#K FOLD CROSS VALIDATION WITH 10 FOLDS
ctrl10 <- trainControl(method = "cv", number = 10)
model10 <- train(stack.loss~Air.Flow+Water.Temp+Acid.Conc., data = stackloss,method ="lm", trControl = ctrl10)
print(model5)
print(model7)
print(model10)

```
RMSE in the output is the square root of the test mse.So squaring the RMSE values for the three folds,k=5,7,10 gives us the average test mse

Average test MSE for k=5=3.838848^2=14.73675

Average test MSE for k=7=3.556371^2=12.64777

Average Test MSE for k=10 folds=3.38069 ^2=11.42906
